...
import { AppMode, Message, ArtifactType, GeneratedArtifact, AppSettings, Alert, ReportTemplate } from './types';
import { Dashboard } from './components/Dashboard';
import { SettingsView } from './components/SettingsView';
import { DocumentsView } from './components/DocumentsView';
import { useLiveSession } from './services/liveService';
import { sendMessageToElsi, generateArtifact } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
     ...
     const checkMetrics = () => {
         ...
         // Mock current values (in real app, read from Dashboard data source)
         const currentRevenue = 4000; 
         const currentExpenses = 2400;

         // We use the current language setting to generate the alert text for immediate feedback
         // In a real app, you might store codes and translate at render time
         ...
         if (newAlerts.length > 0) {
             setAlerts(prev => {
                 const existingIds = new Set(prev.map(a => a.title)); // Dedupe by title for demo
  ...
  // Voice Session
  const { isConnected, isConnecting, volume, connect, disconnect, error } = useLiveSession();
  ...
  const handleSendMessage = async (text: string = inputValue) => {
    ...
    const isArtifactRequest = lowerInput.includes('quote') || lowerInput.includes('plan') || lowerInput.includes('table') || lowerInput.includes('devis') || lowerInput.includes('tableau');
    ...
    try {
        let responseText = '';
        let newArtifact: GeneratedArtifact | undefined;

        if (isArtifactRequest) {
            let type = ArtifactType.REPORT;
            if (lowerInput.includes('quote') || lowerInput.includes('devis')) type = ArtifactType.QUOTE;
            if (lowerInput.includes('plan')) type = ArtifactType.ACTION_PLAN;
            if (lowerInput.includes('table') || lowerInput.includes('tableau')) type = ArtifactType.TABLE;

            newArtifact = await generateArtifact(userMsg.text, type, settings.language);
            if (newArtifact) {
                setArtifacts(prev => [newArtifact!, ...prev]);
                responseText = settings.language === 'fr' 
                   ? `J'ai généré un ${newArtifact.type.toLowerCase()} pour vous.`
                   : `I've generated a ${newArtifact.type.toLowerCase().replace('_', ' ')} for you.`;
            }
        } else {
             responseText = await sendMessageToElsi(userMsg.text, settings.language);
        }

        setMessages(prev => prev.map(m => 
            m.id === loadingId 
            ? { ...m, text: responseText, isThinking: false, artifact: newArtifact } 
            : m
        ));
    } catch (e) {
        setMessages(prev => prev.map(m => 
            m.id === loadingId 
            ? { ...m, text: "Connection error.", isThinking: false } 
            : m
        ));
    }
  };
    ...
    <nav className={`fixed inset-y-0 left-0 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:relative z-50 w-64 bg-slate-900 text-white transition-transform duration-300 ease-in-out flex flex-col`}>
        ...
        <div className="p-4 border-t border-slate-800">
            <div className="flex items-center gap-3">
                <img src="https://picsum.photos/40/40" alt="User" className="w-10 h-10 rounded-full border-2 border-slate-700" />
                <div>
                    <p className="text-sm font-medium">Alex Merchant</p>
  ...
  const renderArtifactCard = (artifact: GeneratedArtifact) => {
    // Reusing existing card logic, simplified for brevity in this update
    return (
        <div className="bg-white border border-slate-200 rounded-lg p-4 my-2 shadow-sm w-full max-w-md">
            <div className="flex items-center gap-2 mb-2">
                <FileText className="text-indigo-500" size={18}/>
                <span className="font-semibold text-slate-800">{artifact.title}</span>
            </div>
            ...
            {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] md:max-w-[70%] rounded-2xl px-4 py-3 shadow-sm ${
                        ...
                        {msg.artifact && renderArtifactCard(msg.artifact)}
                    </div>
                </div>
            ))}
