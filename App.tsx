  ...
  const handleToolCalls = useCallback(async (functionCalls: any[]) => {
      ...
      for (const call of functionCalls) {
          ...
          if (call.name === 'create_quote') {
              const { clientName, items, validDays } = call.args;
              const newQuote: Quote = {
                  id: Date.now().toString(),
                  reference: `Q-${new Date().getFullYear()}-${String(quotes.length + 101)}`, // Simple ref gen
                  status: 'DRAFT',
                  ...
                  terms: { paymentTerms: '30 Days Net', notes: 'Generated by Elsi' }
              };
              setQuotes(prev => [...prev, newQuote]);
              result = { success: true, message: `Quote ${newQuote.reference} created for ${clientName}`, quoteId: newQuote.id };
