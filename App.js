...
import { QuotesView } from './components/QuotesView.js';
import { useLiveSession } from './services/liveService.js';
import { sendMessageToElsi, getChatSession } from './services/geminiService.js';
...
const App = () => {
  ...
  const handleAskAboutAlert = (alert) => {
      setShowAlerts(false);
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
         <div className="bg-white border-b border-slate-200 p-4 flex items-center justify-between sticky top-0 z-20">
             ...
             <div className="hidden md:block font-bold text-lg text-slate-700 ml-4">
                 {t(mode.toLowerCase(), settings.language) || 'ElsyvIA'}
             </div>
